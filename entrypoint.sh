...
_gcloud_log_raw="<pre>$(sed ':a;N;$!ba;s/\n/<br>/g' gcloud.log)</pre><hr><pre>$(sed ':a;N;$!ba;s/\n/<br>/g' traffic.log)</pre>"
_gcloud_log_safe=$(printf '%s' "$_gcloud_log_raw" | tr -d '\n\r')
echo "gcloud_log=${_gcloud_log_safe}" >> "$GITHUB_OUTPUT"

_revision_raw="${SERVICE_NAME}-${REVISION_SUFFIX}"
_revision_safe=$(printf '%s' "$_revision_raw" | tr -d '\n\r')
echo "cloud_run_revision=${_revision_safe}" >> "$GITHUB_OUTPUT"

_endpoint_safe=$(printf '%s' "$ENDPOINT" | tr -d '\n\r')
echo "cloud_run_endpoint=${_endpoint_safe}" >> "$GITHUB_OUTPUT"

_image_tag_safe=$(printf '%s' "$IMAGE_TAG" | tr -d '\n\r')
echo "deployed_image_tag=${_image_tag_safe}" >> "$GITHUB_OUTPUT"
